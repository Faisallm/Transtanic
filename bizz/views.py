...
def detail(request, id):
    ...
    similar_products = Product.objects.filter(tags__in=product_tag_ids).exclude(id=product.id)
    similar_products = similar_products.annotate(same_tags=Count("tags")).order_by("-same_tags", "-created")[:4]
